...
## Operational baseline — what "thousands of clients" means in numbers

**Latency SLA (measured, live):** p50 96ms / p95 165ms / p99 228ms / max 400ms under concurrency-80 load. Target: **p99 < 500ms** — currently 2× inside target.

**Availability / error budget:** 0 × 5xx observed across the load test. The worker self-audits every minute (`/brain/health` → `audit_pass`, `issues[]`), a GitHub Actions uptime monitor watches `self_sustaining`, and `worker-setup.html` shows a version-drift banner. The binding availability constraint is NOT errors — it's the free-tier 100k-req/day cap. **But client-perceived availability > worker availability**, because degradation is graceful:

**Graceful-degradation test (OBSERVED 2026-07-08, not just code-reviewed):** with every `/brain/*` fetch forced to fail (simulating worker-down / free-tier exhaustion), edge-scorecard.html stayed **fully rendered (5.3KB body, no blank/crash)**, showed honest fallbacks ("Could not reach the brain worker", "worker unreachable"), kept last-known values, and threw **zero uncaught console errors**. So a worker outage or free-tier ceiling = a *stale-data window that self-heals*, never a broken site. Error budget for a hard outage is effectively "degraded (stale, labeled) mode," not "down."

**Cost per client (Workers Paid, $0.30/M requests after the $5/mo base):** a client with a tab *visible* 30 min/day ≈ 4 req/min × 30 = 120 requests/day ≈ **$0.0000036/day ≈ $0.0001/month per active client** — a hundredth of a cent. KV reads are colo-cached (shared across the herd) → negligible. **1,000 daily-active clients ≈ the $5/mo base + roughly $1 of usage.** Cost scales with *attention*, not headcount, and is dominated by the flat base fee.

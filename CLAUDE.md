...
- **New pages:** `horizons.html` (multi-horizon: daily/weekly/few-weeks/month/few-months/within-a-year; only the 5-day Weekly card is a graded MODEL call, the rest are honest TREND reads off `/brain/history`), `broadcast.html` (honest X/Twitter composer — daily + long-term + multi-horizon tweet drafts, all "lean … graded vs drift, not advice," manual post via X intent, transparency tweet auto-pulls the live forward-test verdict). Cache-bust is now **v215**; WORKER_VERSION is **pass-304** (added `/brain/history?sym=` — 1y daily closes, KV-cached 6h).

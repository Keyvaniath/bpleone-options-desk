...
> **UPDATE pass 295 (2026-07-01) — the forward-test resolved, and the "different data" thesis was RIGHT.** The legs have separated cleanly (live `/brain/confluence-score`): **🏛 insider open-market buy-clusters n=230, 67.4%, +3.5%/call, z vs selection-adjusted drift +2.94 → a real, statistically-significant free-data edge** (the documented insider-buying anomaly). 🧠 TA/brain alone n=1,211, 53.0% → still no edge (drift). 🐳 confluence n=164, 67.1% → beats drift but does NOT beat insider-alone. **So the edge is the INSIDER DATA, not the model or the "fusion."** CRITICAL honesty fix this pass: the verdict's `worthIt` compared confluence only to brain-only (never insider-only), so it wrongly credited "fusing signals adds edge" and turned the banner green. Fixed to attribute to the best single component — banner now **amber**: *"REAL EDGE, but it's the insider signal, not the model"* (worker verdict + a client safety-net in `edge-scorecard.html`; new verdict fields `insider_edge_found`, `fusion_beats_insider`). **Rule going forward: never let the ML model take credit for the insider edge — that honest attribution IS the MD-credibility thesis** (see `_md-pitch-package.md`). Broadcast picks are also no longer net-negative — drift-consistent / slightly positive now, still labeled "explained by drift, not edge." Options-flow experiment still accruing.
...
- **/brain/signals cross-sectional ranks use MID-RANK for ties** (all-equal probabilities rank 0.5, never 0), and in NO PLAY every name is HOLD with the reason. Ranking noise or ties into BUY/SELL is the failure this prevents.
...
- **Significance requires OVERLAP CORRECTION.** Daily calls on 5-day forward windows are NOT independent — `score()` in /brain/confluence-score clusters same-(sym,dir) calls within 9 calendar days into `effective_n` and scales z by sqrt(n_eff/n); `beats_drift_95` gates on the corrected z AND n_eff ≥ 5. The historical z=+2.94 insider print was a false positive from this exact bug (n=230 was ~25–45 effective). NEVER quote raw n as evidence; edge-scorecard.html shows "independent obs" + a public methodology-correction note — keep both.
- **`/track/stats` feedback + recent are ADMIN-ONLY** (feedback carries user emails; recent exposes per-visitor behavior). Aggregate counters stay public.
- **NEVER edge-cache an upstream FAILURE (pass 308).** Every feed endpoint (`/brain/econ`, earnings, news, halts, premarket, insider*, recommendations, profile, fundamentals, options-*) gates `cache.put` on a verified-good response; failures get `max-age=60` and NO cache write. The unconditional pattern pinned an empty econ calendar for a full hour during CPI week. When adding a feed endpoint, copy the `good`-gated pattern, not the old one.
- **Finnhub earnings must be fetched in ≤7-day chunks (pass 308).** The upstream returns date-DESCENDING capped at 1500 rows — a 14-day single call silently drops TODAY'S reporters on busy weeks (measured: 44 rows). The `/brain/earnings` handler chunks (loop is `d0 <= days`, range inclusive), merges, dedupes by symbol|date, sorts ascending.
- **Picks staleness is per-signal, not per-envelope (passes 307/318).** `/brain/picks` `updatedAt`/`ageSec` stay fresh even when every signal is the prior session's (cron rewrites the SIGNALS wrapper across weekends/pre-open). Clients must read each pick's `stale_scan`/`as_of` (pick-of-day, proof, broadcast do) — never infer freshness from the envelope alone.
- **Narrative rule:** the insider edge didn't "regress" — it was a caught false positive. All pitches/posts use that framing (see `_md-pitch-package.md` 8/10 update, `_no-edge-story-draft.md`).
- **Strategic state (8/10):** ~9 external pageviews total. Audit moratorium recommended until real readership; the next unit of work should be distribution (the no-edge story draft), not another audit pass.

**Pass 305 invariants (the live-data errors Brandon reported — do NOT regress these):**
- **The INITIAL price poll must NOT be visibility-gated.** `worker-quotes.js start()` runs `pollOnce()/pollQuotes()` UNCONDITIONALLY on first load; only the recurring `setInterval(pollCycle)` skips while `document.hidden` (that's the pass-296 scale win). If you ever route the first fetch through `pollCycle()` again, background-tab / prerender loads sit on stale SEED prices showing "MODE: MOCK" until focused.
- **NEVER hardcode a `?v=` version in a dynamic script loader.** Every `document.createElement('script')` loader must derive its cache-bust from its OWN host script's `?v=` (see `live.js` `_lvEager` + `LV`, and `app.js` `CACHE_BUST`). A hardcoded `?v=vNNN` freezes that lazily-loaded file at a cached copy so deployed fixes never reach returning users — this bug class has now bitten TWICE (pass-270 app.js, pass-305 live.js eager loader). Grep `\?v=v[0-9]` in js/ before shipping; the only match should be comments.
- **The data-mode banner must recognize the WORKER feed as live.** `DataModeBanner` treats `window.BPLEONE_DATA_MODE==='live'` (or a real `worker-yahoo`/`finnhub`/`coinbase` quote in QUOTES) as live — NOT just a Finnhub key / DataProvider WS. It re-paints on `bpleone:quotes`/`bpleone:data-mode`. Otherwise it falsely says "No live feed connected / MODE: MOCK" while real delayed prices are on the page. Recognizing the worker feed keeps the honest "equities DELAYED ~15min / crypto LIVE" wording, never a false MOCK.
- **New pages:** `horizons.html` (multi-horizon: daily/weekly/few-weeks/month/few-months/within-a-year; only the 5-day Weekly card is a graded MODEL call, the rest are honest TREND reads off `/brain/history`), `broadcast.html` (honest X/Twitter composer — daily + long-term + multi-horizon tweet drafts, all "lean … graded vs drift, not advice," manual post via X intent, transparency tweet auto-pulls the live forward-test verdict). Cache-bust is now **v218**; WORKER_VERSION is **pass-305** (added `/brain/history?sym=` — 1y daily closes, KV-cached 6h).

**Pass 305b–312 invariants (do NOT regress; all deployed + verified):**
- **The `getQ` honesty rule is now on 12+ pages.** getQ returns `null` (not a `{last:100}` placeholder) when a symbol has no real print; renders show `—`; aggregates count only real quotes; derived views re-render on `bpleone:quotes`. Off-universe symbol lists hydrate real prices via **`js/live-hydrate.js`** `hydrateQuotes(list,{onDone})` (CDN-snapshot-first, then `/brain/quotes`; one non-blocking 2.5s retry). Trade-ledger pages (positions-live, paper-portfolio) must NEVER silently book a fabricated flat close — require an explicit confirm when there's no live quote.
- **Broadcast attribution (pass-295 rule, client-side):** any "our model" claim uses `brain_hit_rate` vs `drift_base_rate`, NEVER `confluence_hit_rate`; if `insider_edge_found && !fusion_beats_insider`, credit the INSIDER DATA, not the model.
- **Worker `/brain/history` (pass-305):** only `UNIVERSE` symbols persist to KV (public GET — off-list symbols are served live + isolate-cached only, so an attacker cycling symbols can't drain the write budget); 404 (not 502) for unknown symbols; expirationTtl 72h backs the serve-stale branch. `/brain/quotes` stale-refresh rotates STALEST-FIRST.
- **Symbol input reaching innerHTML must be strict-sanitized** `replace(/[^A-Z0-9.\-]/g,'').slice(0,8)` (reflected-XSS guard) — trade-coach, day-trader-pro, horizons, broadcast do this.
- **Whole-site quality baselines (keep them at 0-gap):** every page has a self-referential `<link rel="canonical">` + meta description (pass 309); global a11y in css/style.css + app.js — `prefers-reduced-motion`, `:focus-visible`, skip-link → `#main-content`, aria-labels on icon nav buttons (pass 310, WCAG 2.3.3/2.4.1/2.4.7/4.1.2); every external `target="_blank"` carries `rel="noopener"` (pass 311); `fonts.gstatic.com` preconnect on all pages (pass 312). A 128-page live-iframe smoke sweep (pass 308b) is the runtime regression test — 0 uncaught JS errors is the bar.
- **Dark-pool pages:** `generatePrint()` is intentionally hard-disabled (`return null` — no synthetic institutional prints); the `setInterval` callsite MUST guard `if (p) appendPrint(p)` (a null deref there threw every tick).
- **NEVER fabricate a placeholder quote (pass 305b — the getQ bug class).** The idiom `getQ(s){ return QUOTES[s] || { last: 100 } }` (or `{last:0}`, `|| 540`, `q.last || 100`) rendered fake $100.00/$0.00/+0.00% as LIVE on 12 unbannered pages — worst case trade-coach built a full entry/stop/target/sizing plan off a fabricated $100. The rule: `getQ` returns **null** for a symbol without a real print (`q && q.last > 0`), the render shows **—** / "no live quote", aggregates count ONLY real quotes, and derived views re-render on `bpleone:quotes`. For symbol lists with off-universe names, hydrate real prices via the shared **`js/live-hydrate.js`** (`hydrateQuotes(list, {onDone})` — pulls any ticker from `/brain/quotes`, creates QUOTES entries, fires `bpleone:quotes`). Before shipping any page that reads QUOTES, grep it for `\{ last: [0-9]` and `\|\| 100` — the only acceptable placeholder quotes are on pages carrying a SAMPLE/ILLUSTRATIVE banner.

**Pass 299 invariants (from a 28-agent Goldman/scale audit — do NOT regress these):**
- **Honesty grading bar is DRIFT, never 50%.** Any page that shows a letter grade / verdict for the brain (model-results, brain-grade, mobile-dashboard were the offenders) must grade against the market-drift base rate (~52%), never a 50% coin flip, and must NEVER use promissory copy ("press the bet", "solidly profitable", "crushing it", "firing well"). A >50% local hit rate is drift, not skill. When a page can't fetch the drift number, add a caveat + link to edge-scorecard.html rather than implying edge.
- **XSS: escape ALL remote feed strings before innerHTML.** News headlines/sources/URLs (/brain/news) and SEC insider names (/brain/insider-feed) are attacker-influenceable. Every page rendering them (news*, ticker, insider-live, smart-money-confluence) must run text fields through an HTML escaper and validate URLs to `^https?://` before putting them in an href. Never interpolate a raw feed field into innerHTML.
- **Client pollers must be visibility-gated.** Any `setInterval` that hits the network (data-provider Stooq/Coinbase, worker-bridge sync, continuous-learner ticks, worker-quotes) must `return` early when `document.hidden` and catch up on `visibilitychange` — background tabs otherwise poll forever and dominate load at scale.
- **Worker scale invariants (extends pass 296/298):** hot read-only GETs use `kvGet(...,ttl)` + `json(body,200,cacheSec)`; NEVER add a KV write on a GET path unless gated on a dirty flag (see /brain/quotes); write-heavy unauth POSTs (/auth/register, /track) are rate-limited via RL_HITS; admin token compared with `timingSafeEqualHex`, not `!==`.
...
- **Scale invariants (pass 296 — do NOT undo):** hot read-only endpoints use `kvGet(..., 30)` per-colo read caching + `json(body, 200, cacheSec)` Cache-Control (cron rewrites those keys ~1/min, so 30s lag is invisible; NEVER add cacheTtl to read-modify-write paths — journal/model/auth/analytics). `/brain/quotes` is stale-while-revalidate (<15 min stale served immediately, ~1-in-4 refresh) to prevent the Yahoo/KV thundering herd. `recordTrack` SAMPLES pageviews 1-in-5 with counts pre-multiplied ×5 (disclosed in payload) — unsampled pageview writes on the single ANALYTICS key would exhaust the KV write budget the brain's cron needs. Client side: `worker-quotes.js` skips polls while `document.hidden` and uses a per-client jittered 30–45s cadence — a fixed cadence re-creates synchronized request herds. **Abuse (pass 297):** `GET /brain/*` has a worker-native per-IP rate limit (module-scope `RL_HITS` Map, in-memory sliding window, ~120 req/10s → 429) — do NOT swap it for a KV-backed counter (that would defeat the read caching). It lives on `*.workers.dev` which is NOT behind the zone WAF, so a dashboard rate-limit rule wouldn't apply; the worker limiter IS the guard, with Cloudflare's automatic L3/L4 DDoS underneath for distributed attacks.
...
11. **GitHub Pages runs Jekyll on this repo (there is deliberately NO `.nojekyll`).** Three production-only failure modes that local preview can NOT catch: (a) literal Liquid syntax — an open brace immediately followed by another open brace, or by a percent sign — gets processed/stripped on deploy when it appears in pages (a TOTAL-in-double-braces placeholder once shipped blank to production while looking fine locally); (b) Jekyll converts ALL .md files as pages, so invalid Liquid in ANY markdown file FAILS THE WHOLE BUILD and the site silently stays on the previous deploy — this exact convention entry originally spelled the syntax out literally and broke the build (run 27257234547); describe the syntax in words, never write it; (c) `_`-prefixed files at root are EXCLUDED from the deployed site (that's why the `_test_*.js` / `_*.py` dev artifacts stay private — keep them underscore-prefixed, and never reference a `_` file from a page). Before pushing, grep *.html AND *.md for brace-brace / brace-percent sequences, and after pushing check the pages-build-deployment workflow actually succeeded (gh run list).
